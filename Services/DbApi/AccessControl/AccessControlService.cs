...
public class AccessControlService(ApplicationDbContext dbContext, IInventoryRepository inventoryRepo)
{
    ...
    public async Task<bool> CanCreateAsync(Guid inventoryId, ApplicationUser user)
    {
    ...
    public async Task<bool> CanWriteAsync(Guid inventoryId, ApplicationUser user)
    {
        if (user.IsAdmin) return true;
        Inventory? inventory = await _inventoryRepo.GetWithAccessesAsync(inventoryId);
        return inventory != null && inventory.UserHasWriteAccess(user);
    }
}